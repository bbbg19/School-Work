		 ...
		 if(pid == 0){
			...
			while (doneTranslating ==0){			
				...
				if(buffer[0]=='$'){
					...
					write(1,"opt_dec cannot use otp_enc_d\n",30 );
					close(sockfd);
					exit(1);
				}
				...
				//go through read in stream 		
				for(i=0; i<strlen(buffer); i++){
					
					a = buffer[i];
				//	printf("old value: %c.  KeyValue: %c  ",a, keyFile[i]);
					//check inputchar is a valid letter
					if(!isalpha(a) && !isspace(a) && buffer[i]!= '$'){
						write(1,"otp_enc_d error: input contains bad characters\n", 48);
						n = write(newsockfd,"-1",strlen(buffer));
						exit(1);
					}
					...
					//verify this is not a new  line
					else {								
						a=((keyFile[i]-'A') + (buffer[i]-'A'))%27;
						//printf("Value After Mod: %d", a);					
						if(a==26){
							buffer[i]= ' ';							
						}
						else{
